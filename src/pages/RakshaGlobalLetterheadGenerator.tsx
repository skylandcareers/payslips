...
import { Settings2, Printer, FileText, Sparkles, Sofa, Leaf, Sprout } from 'lucide-react';
...
We look forward to receiving your valuable order.`,
      signatoryName: "POSANI KUDALA",
      signatoryTitle: "Partner",
      signatoryFirm: "M/S RAKSHA GLOBAL ENTERPRISES",
    },
  },
  {
    id: "tax-invoice",
...
**Receiver's Signature / Seal:** ________________________`,
  ...
  },
  {
    ...
    data: {
      ...
      bodyText: `With reference to the subject cited above, we, **M/S RAKSHA GLOBAL ENTERPRISES** (GSTIN: 36ABMFR7406E1Z2), a registered partnership firm operating in the wholesale and retail trade of agricultural products, are hereby submitting our required documents.
...
My son, Mr. RAGHU KUDALA, has been accepted to present his doctoral research paper at the international biotechnology conference BIOSPAIN 2026, scheduled to take place in Bilbao, Spain from September 29th to October 1st, 2026.
...
My son is traveling strictly for academic purposes. He has strong academic commitments in India and will return immediately after completing his conference visit to resume his Ph.D. research at SASTRA University.

All necessary financial and business credentials, including GST registration certificate, PAN, bank statements, Income Tax Returns, certificates from SASTRA University, and BIOSPAIN 2026 receipts, are submitted in support of this application.
...
const RakshaGlobalLetterheadGenerator = () => {
  ...
  const [headerData, setHeaderData] = useState({
    firmName: 'M/S RAKSHA GLOBAL ENTERPRISES',
    tagline: 'Trader - Retailer | Wholesale & Retail Business',
    proprietor: 'POSANI KUDALA',
    gstin: '36ABMFR7406E1Z2',
    hsnCode: 'HSN: 07129020 (Dehydrated Garlic) | 09041150 (Green Pepper)',
    regType: 'REGULAR TAXPAYER (PARTNERSHIP)',
    address: '4-5-24/439, Sundaraiah Nagar, Khursheed Nagar, Adilabad, Telangana - 504001',
    phone: '+91 7428730894',
    email: 'info@rakshaglobal.in',
  });
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-b border-slate-200 print:hidden shadow-sm">
        <div className="container mx-auto px-6 py-3 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-green-700 to-green-900 flex items-center justify-center text-amber-500 font-black text-xl shadow border border-green-800">
              <Sprout className="w-6 h-6 text-amber-500" />
            </div>
            <div className="flex flex-col">
              <span className="text-base font-bold text-slate-900 tracking-tight leading-none">M/S RAKSHA GLOBAL ENTERPRISES</span>
              <span className="text-xs text-green-800 font-semibold uppercase tracking-wider">GSTIN: {headerData.gstin} • Official Generator</span>
            </div>
          </Link>
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => window.print()} className="inline-flex items-center gap-2 text-white px-5 py-2.5 text-xs font-bold rounded-lg shadow hover:bg-green-900 transition-colors bg-green-800">
        ...
        <div className={`w-full md:w-[380px] bg-white border border-slate-200 rounded-xl shadow-sm p-6 flex-col gap-6 overflow-y-auto max-h-[calc(100vh-120px)] sticky top-24 print:hidden shrink-0 ${activeTab === "form" ? "flex" : "hidden md:flex"}`}>
          ...
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <div className="flex items-center gap-2">
              <Settings2 className="w-4 h-4 text-green-800" />
          ...
          {/* Template Selectors */}
          <div>
            <label className="block text-[11px] mb-2 text-green-800 font-extrabold uppercase tracking-wider flex items-center gap-1">
            ...
            <div className="grid grid-cols-1 gap-2">
              {templateOptions.map((t) => (
                ...
                >
                  <span>{t.name}</span>
                  <FileText className="w-4 h-4 text-green-800 group-hover:scale-110 transition-transform" />
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="pt-2 border-t border-slate-100 space-y-3">
            <div>
              ...
              <select
                ...
                className={`${inputCls} cursor-pointer font-bold text-green-800 bg-blue-50/60 border-blue-200`}
              >
        ...
        <div className={`print-container flex-1 overflow-y-auto bg-slate-200/50 p-4 md:p-8 rounded-2xl md:flex flex-col gap-10 items-center relative print:block print:bg-transparent print:rounded-none print:p-0 print:m-0 ${activeTab === "preview" ? "flex" : "hidden"}`}>
          ...
          <div className="page-container page w-full max-w-[210mm] min-h-[297mm] bg-white text-black shadow-2xl px-8 py-8 md:px-12 md:py-10 relative flex flex-col print:shadow-none print:w-full print:max-w-none" style={{ fontFamily: selectedFont }}>
            ...
            <div className="w-full flex flex-col">
              ...
              <div className="w-full pb-4 mb-5 relative flex items-start justify-between">
                
                {/* Sleek Corporate Logo Block - Scaled up */}
                <div className="flex-shrink-0 mt-0.5">
                  <div className="w-24 h-24 bg-green-700 rounded-sm flex flex-col items-center justify-center relative overflow-hidden shadow-sm">
                    <div className="absolute top-0 left-0 w-full h-2 bg-slate-900/30"></div>
                    <span className="text-[40px] font-black text-white tracking-widest leading-none font-serif mt-2 ml-1.5 drop-shadow-sm">RGE</span>
                    <span className="text-[8px] font-extrabold text-green-100 uppercase tracking-[0.4em] mt-1 ml-1">Global</span>
                  </div>
                </div>
                ...
                <div className="flex flex-col items-start text-left flex-1 pt-0">
                  {/* Firm Name - Forced single line, larger text */}
                  ...
                  {/* Tagline - Scaled up */}
                  <p className="text-[12px] font-bold text-green-700 tracking-[0.2em] uppercase mb-3.5">
                    {headerData.tagline}
                  </p>
                  ...
                  {/* Contact Row 1 (GSTIN, Email, Phone) - Scaled up */}
                ...
                <div className="absolute bottom-0 left-0 right-0">
                  <div className="h-[2px] w-full bg-green-700 mb-[2px]"></div>
              ...
              {/* Subject */}
              {formData.subject && (
                <div className="border-l-[3.5px] border-amber-500 bg-green-50/60 p-2 mb-2 font-sans text-[9.2pt] leading-snug font-bold text-green-950">
              ...
              {/* Body Content */}
              <div className="w-full text-[9.6pt] leading-[1.6] text-justify text-slate-900 mb-2 [&_p]:mb-3 prose-strong:font-bold prose-strong:text-green-950">
            ...
            <div className="w-full mt-1 pt-1 flex flex-col items-start text-left font-sans">
              ...
              <div className="text-[10.5pt] font-bold text-green-950 uppercase">{formData.signatoryName || headerData.proprietor}</div>
              <div className="text-[9pt] font-semibold text-slate-700">{formData.signatoryTitle || 'Partner'}</div>
            </div>

            {/* Centered Page Footer */}
            <footer className="print-fixed-footer w-full font-sans mt-auto pt-3.5 pb-1.5 border-t-2 border-green-900 bg-white text-center text-[9px] text-slate-900 leading-snug">
...
export default RakshaGlobalLetterheadGenerator;
